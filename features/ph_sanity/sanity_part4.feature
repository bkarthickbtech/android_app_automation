...
Feature: New user checkout with new address and voucher
	...
	Scenario: Checkout new user with voucher
		And  I wait up to 30 seconds for "Philippines" to appear
		And  I touch the "Philippines" text
		...
		And  I wait up to 30 seconds for "Women - Wallets" to appear
		And  I touch the "Women - Wallets" text
		...
		Then I choose billing province as "Metro Manila"
		Then I choose billing city as "Manila"
		And  I scroll down
		...
		Then I take a screenshot with name "ph_order_confirmation_new_voucher"
		Then I wait for final loading to be complete
	    ...
	    Then I take a screenshot with name "ph_order_number_new_voucher"
		And  I ensure that no account is logged in for "en"

	@newusercheckoutnewaddress @checkout
	Scenario: Checkout new user with new address
		And  I check for country select "Philippines"
		Then I wait up to 30 seconds for the view with id "imgTop" to appear
		And  I ensure that no account is logged in for "en"
		And  I ensure that the cart is empty for "en"
		And  I wait for 3 seconds
		...
		Then I drag from 50:80 to 50:20 moving with 5 steps
		And  I add a new address at checkout for ph
		Then I wait for loading to be complete
		And  I wait up to 30 seconds for "button" to appear in webview
		Then I wait for final loading to be complete
		...
		And  I wait up to 30 seconds for "button" to appear in webview
		Then I wait for final loading to be complete
		Then I enter billing first name "new_address"
		Then I enter billing last name "test"
		Then I enter billing cell no 65498765
		And  I enter billing mobile no 9874561235
		Then I enter billing postcode 123456
		Then I enter billing address1 info "new address test"
		Then I enter billing address2 info "new address"
		Then I choose billing province as "Metro Manila"
		Then I choose billing city as "Manila"
		And  I scroll down
		And  In webview I click "button" once
		...
		And  I wait up to 30 seconds for "button" to appear in webview
		Then I wait for final loading to be complete
		Then I take a screenshot with name "ph_multiple_addresses"
		And  I select addresses
		And  In webview I click "button"
	    And  I wait up to 30 seconds for "button" to appear in webview
		Then I wait for final loading to be complete
		Then I take a screenshot with name "ph_order_confirmation_new_address"
		And  In webview I click "div.txtCenter.pbl button.btn.cta.js_checkoutBtn"
	    Then I wait up to 60 seconds for "Thank you for your order!" to appear
	    Then I take a screenshot with name "ph_order_number_new_address"
		And  I ensure that no account is logged in for "en"