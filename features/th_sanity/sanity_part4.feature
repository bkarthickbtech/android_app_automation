...
Feature: New user checkout with new address and voucher
	...
	Scenario: Checkout new user with voucher
		...
		Then I take a screenshot with name "th_order_confirmation_new_voucher"
		Then I wait for final loading to be complete
	    ...
	    Then I take a screenshot with name "th_order_number_new_voucher"
		And  I ensure that no account is logged in for "th"
		...
		Then I wait for final loading to be complete
		#LANGUAGE CHANGE
		Then I touch the "เพิ่มที่อยู่" text in webview
		Then I wait for loading to be complete
		And  I wait up to 30 seconds for "button" to appear in webview
		Then I wait for final loading to be complete
		#START LANGUAGE CHANGE
		And  I enter shipping mobile no 0814569835
		And  I enter shipping cell no 0814569835
		And  I enter shipping address1 info "#07- Block 402 B"
		And  I enter shipping address2 info "punggol field"
		And  I enter shipping city info "Bang na"
		Then I choose shipping province as "กาญจนบุรี"
		And  I enter shipping postcode 87565
		#END LANGUAGE CHANGE
		And  I scroll down
		And  In webview I click "button" once
		And  In webview I click "button" once
		And  I wait up to 30 seconds for "button" to appear in webview
		Then I wait for final loading to be complete
		Then I take a screenshot with name "th_multiple_addresses"
		And  I select addresses
		And  In webview I click "button"
	    Then I wait for loading to be complete
	    And  I wait up to 30 seconds for "button" to appear in webview
		Then I wait for final loading to be complete
		Then I take a screenshot with name "th_order_confirmation_new_address"
		And  In webview I click "div.txtCenter.pbl button.btn.cta.js_checkoutBtn"
		#LANGUAGE CHANGE
	    When I wait up to 60 seconds for "ขอบคุณที่ใช้บริการค่ะ :)" to appear
	    Then I take a screenshot with name "th_order_number_new_address"
		And  I ensure that no account is logged in for "th"